<!--
Copyright (c) Office FA.com co., Ltd. All Rights Reserved.
Created by : AnhNP
Created date : 2018/06/12
Updated by :
Updated date :
 -->
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/resources/css/front/65/65.css" />
<!--contant-->
<div id="div_contant" class="div_contant">
	<div class="div_contant_left">
		<div class="div_contant_right_1 no-body">
			<div class="div_contant_right_1_2">
				<div class="div_contant_right_1_2_1 ten_bang">絞り込み項目</div>
			</div>
			<div class="div_contant_right_1_3">
				<table class="table_right_1_1 chu_trong_bang">
					<tbody>
						<tr>
							<td class="text_left">評価プロセスの名称</td>
							<td class="text_right">XXXXXXXX</td>
						</tr>
						<tr>
							<td class="text_left">入力する評価</td>
							<td class="text_right">業績目標達成度</td>
						</tr>
						<tr>
							<td class="text_left">所属本部</td>
							<td class="text_right">〇〇本部</td>
						</tr>
						<tr>
							<td class="text_left">部署</td>
							<td class="text_right">〇〇部署</td>
						</tr>
						<tr>
							<td class="text_left">等級</td>
							<td class="text_right">XX</td>
						</tr>
						<tr>
							<td class="text_left">キャリアパス</td>
							<td class="text_right">XX</td>
						</tr>
						<tr>
							<td class="text_left">提出版の総合評価</td>
							<td class="text_right">XX</td>
						</tr>
					</tbody>
				</table>
			</div>
			<div style="clear: both"></div>
		</div>

	</div>
	<div class="div_contant_right">
		<div class="tab chu_trong_bang">
			<button class="tablinks firt" onclick="openCity(event, 'London')">田中&nbsp;&nbsp;
				一郎</button>
			<div class="div_tab"></div>
			<button class="tablinks" onclick="openCity(event, 'Paris')">田中&nbsp;&nbsp;
				一郎</button>
			<div class="div_tab"></div>
			<button class="tablinks" onclick="openCity(event, 'Tokyo')">田中&nbsp;&nbsp;
				一郎</button>
			<div class="div_tab"></div>
			<button class="tablinks" onclick="openCity(event, 'Tokyo')">田中&nbsp;&nbsp;
				一郎</button>
			<div class="div_tab"></div>
			<button class="tablinks" onclick="openCity(event, 'Tokyo')">田中&nbsp;&nbsp;
				一郎</button>
			<div class="div_tab"></div>
			<button class="tablinks" onclick="openCity(event, 'Tokyo')">山田&nbsp;&nbsp;
				花子</button>
			<div class="div_tab"></div>
			<button class="tablinks" onclick="openCity(event, 'Tokyo')">山田&nbsp;&nbsp;
				花子</button>
			<div class="div_tab"></div>
			<button class="tablinks active" onclick="openCity(event, 'Tokyo')">田中&nbsp;&nbsp;
				一郎</button>
		</div>
		<div class="div_contant_right_2">
			<div class="div_contant_right_1_2">
				<div class="div_contant_right_1_2_1 ten_bang">評価の入力</div>
			</div>
			<div class="div_contant_right_1 no-body">
				<div class="div_contant_right_1_3">
					<table class="table_right_1_3 chu_trong_bang">
						<tr>
							<td class="td_1">社員番号</td>
							<td class="td_4">123456789</td>
						</tr>
						<tr class="tr_1">
							<td class="td_1">最終版の総合評価</td>
							<td class="td_4">XXXXXXXX</td>
						</tr>
						<tr>
							<td class="td_1">最終評価</td>
							<td class="td_4"><input type="text"
								class="input_text text_right" /></td>
						</tr>
					</table>
				</div>
				<div style="clear: both"></div>
			</div>
		</div>
	</div>
	<div style="clear: both"></div>
</div>
<script type="text/javascript">
	$(function() {
		set_resize();
	});
</script>
<!--/contant-->